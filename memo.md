...
- [x] **11. BOM 시각화 줄바꿈 5개 단위 변경 및 같은 라인 간격 50% 축소 (2026-09-06)**
  - [x] **줄바꿈 기준 변경 (8개 ➔ 5개, 6개부터 다음 줄로 줄바꿈)**:
    - `extraLeft = index % 5` 및 `childRow = Math.floor(index / 5)`를 적용하여 한 행에 최대 5개의 부품 카드만 배치되고, 6번째 부품부터 다음 줄의 동일 열(col) 아래로 자동 줄바꿈
  - [x] **같은 라인 종속성 표시를 위한 위 카드와의 세로 간격 50% 축소**:
    ...
    - 카드가 겹치지 않고 온전히 표시되면서도, 시각적으로 상위 라인과 한 묶음임이 직관적으로 드러나도록 최적화

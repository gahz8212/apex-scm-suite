...
#### ⏳ 1~3단계: Upstream 파이프라인 (기획 완료 / 개발 대기)
> **구조적 특징**: 무역/제조 SCM의 단방향 데이터 흐름 특성상, 1·2·3번은 하류(4·5번)의 복잡한 물류/스케줄 로직에 **영향을 받지 않고 독립적으로 설계 및 구현**됩니다.
1. **1단계: Plan (MRP 자재 소요량 계산기)**:
   - **BOM(자재명세서)**의 1대당 소요량(`point`)과 **발주서(Ordersheet)**의 월별 완제품 오더 수량을 수학적 곱연산으로 전개
   - 수백 개 부품의 월별 순소요량을 매트릭스 표로 자동 산출 (공용 부품 자동 합산)
2. **2단계: Source (원클릭 구매/발주 관리 - RFQ/PO)**:
   - 산출된 자재 소요량을 바탕으로 공급처별 단가/납기 입력 및 발주서(PO) 발행
3. **3단계: Store (가상 창고 수불 및 불량/Scrap 관리)**:
   - 입고 자재 적재 및 공정별 불량 발생 시 실시간 재고 차감 (추후 4번 출하 시 일괄 차감 연동)

---

### 🎨 UI/UX 디자인 일괄 정돈 방침 (★합의 완료)
- 기능 개발 도중 화면마다 스타일을 개별 수정하지 않고, **1~5단계 핵심 기능 및 업무 흐름 확정 후 마지막에 통일된 B2B 디자인 토큰(테마/공통 컴포넌트)으로 일괄 정돈**하여 중복 작업을 방지하고 시각적 완성도를 극대화합니다.

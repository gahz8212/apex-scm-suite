...
4. **아이콘 및 이모지 전면 사용 금지 (★절대 원칙)**
   - 견적요청서, 발주서, 입고 모달 등 모든 팝업 모달, 폼 라벨, 액션 버튼, 헤더 타이틀에 일체의 이모지 및 아이콘을 집어넣지 않는다.
   - 앞으로 추가 및 생성되는 모든 UI 요소와 컴포넌트는 군더더기 없는 순수 텍스트(한글/영문)와 단정한 B2B CSS 스타일(예: CSS 라디오 도트 등)로만 구성한다.
...
- [x] **11. BOM 시각화 한 줄 최대 5개(상위 1개 + 부품 4개) 및 6번째부터 2열 줄바꿈 (2026-09-06)**
  ...
  - [x] **다중 행 줄바꿈 주기 및 배치 구조 규격 확정**:
    - **1행 (총 5개)**: 1번째 (상위 SET/ASSY 1열) + 2~5번째 (부품 1~4번, 2~5열)
    - **2행 (총 4개)**: 6~9번째 (부품 5~8번, 2~5열)
    - **3행 (총 4개)**: **10번째 아이템** (부품 9번, `index = 8`)부터 3행 2열로 줄바꿈 ➔ 10~13번째 (부품 9~12번, 2~5열)
    - **4행 이후**: 4개 단위 순환(`14번째`, `18번째`, `22번째`...)으로 2열 시작 수직 정렬 자동 줄바꿈
  - [x] **같은 라인 종속성 표시를 위한 위 카드와의 세로 간격 50% 축소**:
    - 다른 어셈블리/라인 간의 세로 카드 여백(45px)과 달리, 같은 어셈블리 내에서 줄바꿈된 부품 행은 바로 위 부품 행과의 카드 간 세로 여백을 공식(`101 * childRow`)에 따라 정확히 **22.5px**(절반)로 밀착 배치하여 같은 세트 소속임을 직관적으로 표현
...
- [x] **14. Item Master 메뉴 전용 카드 뒷면 견적요청(RFQ) 및 발주요청(PO) 버튼 복원 (2026-09-07)**
  - [x] **Item Master 전용 조달 버튼 분기 (`isItemMaster === true`)**:
     - BOM Management 메뉴(`!isItemMaster`)는 기존의 초간소화된 **`[현재고 | 안전재고]` 2열 지표 및 `[ 앞면 ]` 복귀 버튼**을 그대로 유지하여 화면 복잡도 원천 차단
     - Item Master 메뉴(`isItemMaster = true`)에만 카드 뒷면 하단에 조달 액션 버튼 및 복귀 버튼 배치
  - [x] **원클릭 모달 및 실시간 상태 연동**:
     - `[ 견적요청 ]` 클릭 시 복수 거래처 비교 견적 팝업 모달(`rfqModalItem`) 오픈 및 발송 처리, 발송 완료 시 `[ 발주요청 ]` 전환
     - `[ 발주요청 ]` 클릭 시 정식 발주서 팝업 모달(`poModalItem`) 오픈 및 전송 처리, 발송 완료 시 `[ 입고대기 ]` 전환
- [x] **15. 단일 조달 라이프사이클 버튼 및 입고(Inbound) 재고 합산 자동화 (2026-09-07)**
  - [x] **단일 동적 라이프사이클 버튼 (견적요청 ➔ 발주요청 ➔ 입고대기 ➔ 견적요청 순환)**:
    - **1단계 (기본값)**: 카드 뒷면에 **`[ 견적요청 ]`** 단일 버튼(블루) 표시 ➔ 클릭 시 견적요청서(RFQ) 팝업 모달 오픈
    - **2단계 (견적서 출력/발송 완료 시)**: 버튼 텍스트가 **`[ 발주요청 ]`** (에메랄드)으로 자동 전환 ➔ 클릭 시 정식 발주서(PO) 팝업 모달 오픈
    - **3단계 (발주서 출력/전송 완료 시)**: 버튼 텍스트가 **`[ 입고대기 ]`** (앰버/골든오렌지)으로 자동 전환 ➔ 클릭 시 자재 입고 등록 팝업 모달 오픈
    - **4단계 (창고 및 입고수량 입력 ➔ 재고 합산 및 견적요청 리셋)**:
      - 입고 모달에서 대상 창고(`제1 중앙물류창고`, `제2 부품자재창고` 등) 및 확정 입고 수량을 입력
      - `[ 입고 확정 및 재고 합산 ]` 클릭 시 해당 품목의 현재고(`stock`)에 입력 수량이 즉시 합산(`stock + qty`)되고 DB 및 Redux 스토어에 동시 반영
      - 상태가 `IDLE`로 자동 리셋되어 버튼 이름이 다시 기본값인 **`[ 견적요청 ]`**으로 완벽하게 복귀!
  - [x] **백엔드 & 프론트엔드 풀스택 파이프라인 연동**:
    - 백엔드 라우터 `PATCH /item/inbound` 신설 (DB `stock` 합산 및 `rfq_status = 'IDLE'` 리셋 처리)
    - 프론트엔드 `itemAPI.ts` (`inboundItem`) & Redux `itemSlice.ts` (`inboundItemStock`) 연동
    - `CardContainer.tsx`에서 `onInboundStock` 콜백을 바인딩하여 품목 목록 실시간 재계산 트리거
- [x] **16. 모달/버튼 일체 아이콘 전면 제거 및 순수 B2B 텍스트 표준화 (2026-09-07)**
  - [x] **견적/발주/입고 모달 및 카드 버튼 아이콘 영구 삭제**:
    - 카드 뒷면 라이프사이클 버튼(`입고대기`, `발주요청`, `견적요청`, `앞면`) 내 이모지/아이콘 전면 제거
    - 견적요청서(RFQ), 정식 발주서(PO), 자재 입고(Inbound) 모달의 헤더 타이틀, 라벨, 안내문구, 액션 버튼 일체의 이모지 삭제
    - 라디오 선택 요소를 이모지 대신 세련된 B2B CSS 원형 도트(`.radio-dot`)로 전면 교체
  - [x] **향후 생성 UI 절대 원칙**: 앞으로 추가되는 모든 UI 및 모달에 아이콘/이모지를 일체 삽입하지 않고 순수 텍스트와 정갈한 B2B CSS 스타일로만 설계 준수
- [x] **17. MOQ 배수 스텝 증감(+/- 원형 버튼) & 부분 입고/발주 잔여 대기 파이프라인 완성 (2026-09-07)**
  - [x] **견적요청(RFQ) 수량 증감**:
    - `권장 견적요청 수량` ➔ `견적요청수량`으로 명칭 간소화 및 `(MOQ 반영)` 문구 제거
    - 수량 좌우에 원형 테두리가 있는 `+`, `-` 버튼(`.btn-circle-step`) 배치
    - 클릭 시 `MOQ` 단위로 배수 증감 (최소 MOQ 보장)
  - [x] **정식 발주(PO) 수량 증감**:
    - `발주 확정 수량` ➔ `발주 수량`으로 명칭 간소화 및 `(MOQ: ...)` 문구 제거
    - 수량 좌우에 원형 테두리가 있는 `+`, `-` 버튼 배치 및 `MOQ` 배수 단위 증감 연동
    - 발주 확정 시 발주 수량(`po_qty`)을 DB 및 스토어에 안전하게 보존
  - [x] **자재 입고(Inbound) 발주 수량 자동 세팅 및 불필요 버튼 정리**:
    - `확정 입고 수량` ➔ `입고수량`으로 명칭 변경
    - 입고 팝업 오픈 시 발주했던 수량(`po_qty`)이 입고수량 기본값으로 자동 바인딩
    - 하단 빠른 수량 버튼(`+50`, `+100` 등 칩스) 전면 삭제
  - [x] **부분 입고(Partial Inbound) 스마트 판정 및 발주 잔여 대기 로직**:
    - `입고수량 >= 발주수량`: 전량 입고 완료 ➔ 재고 합산(`stock + qty`) 및 발주 종결 ➔ 카드 버튼이 기본값 `견적요청`으로 복귀
    - `입고수량 < 발주수량`: 입고 완료 여부 확인창 팝업
      - `확인(완료)` 선택: 이번 입고로 발주 종결 처리 ➔ 재고 합산 및 `견적요청` 버튼 복귀
      - `취소(대기)` 선택: 입고 수량만큼 재고 합산, 잔여 수량(`발주수량 - 입고수량`) 자동 계산 및 보존 ➔ 카드 버튼은 **`입고대기`** 상태로 유지!

- [x] **18. 견적 폼 조건부 납기 표시 & 발주 폼 리드타임 1주 단위 가감 스텝 버튼 연동 (2026-09-07)**
  - [x] **견적요청서 폼 조건부 납기 메타 표기**:
    - 공급처 목록에서 최근 납기 정보가 있으면 `기준단가: ￦7,800 / 납기: 2주 / MOQ: 500` 로 온전하게 표시
    - 최근 납기 정보가 없거나 미정이면 `납기` 항목을 생략하고 `기준단가: ￦7,800 / MOQ: 500` 형태로 깔끔하게 표시
  - [x] **발주서 폼 최근 납기 기준 1주 단위 가감 스텝(+/-) 버튼**:
